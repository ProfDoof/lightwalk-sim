...
class WaveDownGradient : public StandardEffect {
public:
  WaveDownGradient(int id, long startTime, int r1, int g1, int b1, int r2, int g2, int b2, int v) : StandardEffect(id, startTime) {
    _gradient.configure(r1, g1, b1, r2, g2, b2);
    _velocity = v;
  }

  bool cares(int x, int y) {
    int relativeY = PIXEL_COUNT - y;
    float localPercentage = relativeY * 1.0 / PIXEL_COUNT;
